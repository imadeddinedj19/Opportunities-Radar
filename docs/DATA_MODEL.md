...
| **FeatureSnapshot** | S2 | One model feature for a company at a date (FR-06) | `company_id`, `snapshot_date`, `feature_name`, `value`, `feature_version` |
| **Score** | S3 | Opportunity score 0-100 (FR-07/08) | `company_id`, `model_version`, `score`, `confidence`, `rank` |
| **ProductRelevance** | S3 | Relevant SIX product family (FR-09) | `company_id`, `product_family`, `relevance_score`, `evidence` |
| **Explanation** | S3 | Human-readable reasons + evidence (FR-10/11) | `company_id`, `reason`, `supporting_features`, `source_ids` |
