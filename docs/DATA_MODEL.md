...
`company_id` and `event_id` are **content-derived** (a hash of the normalized name / of the
company+connector+url). This is what makes the pipeline idempotent: the same input always yields
the same id, so re-running upserts in place instead of creating duplicates.
